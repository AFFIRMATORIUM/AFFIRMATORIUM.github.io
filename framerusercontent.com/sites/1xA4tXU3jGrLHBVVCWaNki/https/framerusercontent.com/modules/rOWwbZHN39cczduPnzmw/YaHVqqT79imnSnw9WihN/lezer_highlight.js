...
var Tag = class _Tag {
    ...
    static define(parent) {
        ...
        tag.set.push(tag);
        if (parent)
            for (let t2 of parent.set) tag.set.push(t2);
        return tag;
    }
    /**
    ...
    static get(base, mods) {
        ...
        for (let parent of base.set)
            if (!parent.modified.length)
                for (let config of configs) set.push(_Modifier.get(parent, config));
        return tag;
    }
    constructor() {
        this.instances = [];
...
function tagHighlighter(tags2, options) {
    ...
    for (let style of tags2) {
        if (!Array.isArray(style.tag)) map[style.tag.id] = style.class;
            ...
            for (let tag of style.tag) map[tag.id] = style.class;
    }
    let {
        scope,
        all = null
...
var HighlightBuilder = class {
    ...
    highlightRange(cursor, from, to, inheritedClass, highlighters) {
        ...
        if (mounted && mounted.overlay) {
            ...
            } while (cursor.nextSibling()) cursor.parent();
        }
    }
